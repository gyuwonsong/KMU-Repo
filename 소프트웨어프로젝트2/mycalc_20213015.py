...
class Calculator(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 버튼 생성 코드 반복 줄이기
        for i in range(0, len(self.digitButton)):
        ...
        # 버튼 배치 코드 반복 줄이기
        for j in range(0, len(self.digitButton)):
            if j == 0 :
                numLayout.addWidget(self.digitButton[j], 3, 0)
            else :
                row = int((9 - j) / 3)
                col = ((j - 1) % 3)
                numLayout.addWidget(self.digitButton[j], row, col)
    ...
    def buttonClicked(self):
        ...
        if key == '=':
            try:
                ...
                QMessageBox.warning(self, '올바르지 않은 입력 형식', "입력을 초기화 합니다.")
                self.display.setText('')
            except ZeroDivisionError:
                QMessageBox.warning(self, '0으로 나눌 수 없음', "입력을 초기화 합니다.")
                self.display.setText('')
        elif key == 'C':
            self.display.setText('')
        else:
